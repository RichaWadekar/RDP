...
test('Rainyday Admin Login via Yopmail OTP - Standalone', async ({ browser }) => {
  ...
  try {
    ...
    if (found) {
      ...
      await sleep(3000);
      try { const p = appContext.pages()[0]; if (p) await p.screenshot({ path: 'test-results/screenshots/login-success-stage.png', fullPage: true }).catch(()=>{}); } catch(e){}
      ...
      try { const p = appContext.pages()[0]; if (p) await p.screenshot({ path: 'test-results/screenshots/login-fallback.png', fullPage: true }).catch(()=>{}); } catch(e){}
      if (mailContext) await mailContext.close().catch(()=>{});
      if (appContext) await appContext.close().catch(()=>{});
      return;
    }
    ...
    try {
      await appPage.screenshot({ path: 'test-results/screenshots/login-failed-stage.png', fullPage: true });
    } catch (e) {}
    throw error;
  } finally {
    if (mailContext) {
