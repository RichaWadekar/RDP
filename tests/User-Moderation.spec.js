...
test('User Moderation - Complete Workflow (Filters + Warning + Ban + Ignore)', async ({ browser }) => {
  ...
  try {
    ...
    if (await rrButton.count() > 0) {
      ...
      await page.waitForTimeout(300);
      await page.screenshot({ path: 'filter-open-reported-reason.png' }).catch(() => {});
  ...
  if (!rrOk) {
    ...
    if (await rrLabel.count() > 0) {
      ...
      await page.screenshot({ path: 'reported-reason-failed-page.png', fullPage: true }).catch(() => {});
      try {
  ...
  try {
    ...
    if (await stButton.count() > 0) {
      ...
      await page.waitForTimeout(400);
      await page.screenshot({ path: 'filter-open-status.png' }).catch(() => {});
  ...
  if (!rrOk) {
    await page.screenshot({ path: 'reported-reason-not-selected.png', fullPage: true }).catch(() => {});
    throw new Error('Reported Reason selection failed; aborting test');
  }
  if (!stOk) {
    await page.screenshot({ path: 'status-not-selected.png', fullPage: true }).catch(() => {});
